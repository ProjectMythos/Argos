...
import net.projectmythos.argos.utils.HttpUtils;
import net.projectmythos.argos.utils.ItemBuilder;
...
import static net.projectmythos.argos.utils.UUIDUtils.uuidFormat;
import static net.projectmythos.argos.utils.UUIDUtils.uuidUnformat;
...
public class SkinCache implements PlayerOwnedObject {
	...
	public boolean update() {
		...
		try {
			...
			Argos.warn("Error getting " + getNickname() + "'s current player profile: " + ex.getMessage());
			return false;
		}
	...
	private @NotNull ProfileProperty getTextureProperty(PlayerProfile profile) {
		...
		throw new ArgosException("No texture property:\n" + StringUtils.toPrettyString(profile));
	}
