package net.projectmythos.argos.models.cooldown;
...
public class CooldownService extends MongoPlayerService<Cooldown> {
	 ...
	 */
	public boolean check(OfflinePlayer player, String type, TimeUtils.TickTime time) {
	 ...
	 * @return true if player is not on cooldown
	 */
	public boolean check(UUID uuid, String type, TimeUtils.TickTime time) {
	...
	public boolean check(UUID uuid, String type, long ticks, boolean createIfTrue) {
		...
		if (cooldown == null) {
			Argos.warn("Cooldown object is null? " + uuid.toString() + " / " + type + " / " + ticks);
			return false;
		}
